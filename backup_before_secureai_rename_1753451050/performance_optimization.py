...
def test_basic_redaction_performance():
    ...
    try:
        ...
        for test_case in test_cases:
            ...
            result = redactor.redact_text(test_case["content"], use_ai=False)
            end_time = time.time()
...
def test_memory_usage():
    ...
    try:
        ...
        start_time = time.time()
        result = redactor.redact_text(large_content, use_ai=False)
        end_time = time.time()
...
def test_concurrent_processing():
    ...
    try:
        ...
        def process_text(text_id, content):
            start_time = time.time()
            result = redactor.redact_text(content, use_ai=False)
            end_time = time.time()
            return {
                "thread_id": text_id,
                "processing_time": end_time - start_time,
            ...
            ("config.json", """
{
    ...
    "api": {
        ...
        for file_path in test_files:
            start_time = time.time()
            result = redactor.redact_file(str(file_path))
            end_time = time.time()
...
def generate_optimization_report(results):
    ...
    if results.get("basic_redaction"):
        ...
        if avg_chars_per_second < 10000:
            print("⚠️  Consider optimizing regex patterns for better performance")
        else:
    ...
    if results.get("memory_usage"):
        ...
        if memory_data["memory_increase_mb"] > 100:
            print("⚠️  High memory usage detected. Consider processing in smaller chunks")
        else:
    ...
    if results.get("concurrent_processing"):
        ...
        if concurrent_data["total_time"] > concurrent_data["average_time"] * 2:
            print("⚠️  Concurrent processing shows overhead. Consider sequential processing for small files")
        else:
