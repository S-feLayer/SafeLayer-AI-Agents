# Windows Setup Script for secureai MCP
...
if (-not $SkipInstall) {
    ...
    try {
        ...
        Write-Host "✅ secureai installed successfully" -ForegroundColor Green
    }
    catch {
        Write-Host "❌ Failed to install secureai: $_" -ForegroundColor Red
        exit 1
    }
...
if (-not (Test-Path $envFile)) {
    $envContent = @"
# secureai Environment Variables
